...
--	if not removefunc then return end
	...
	--	print(
		if spell and not BanSpells[spell] then
			local basecd = MyGetSpellBaseCooldonw(spell)
			local spellid = spell
			texture  = GetSpellTexture(spell);
			spell = FW:SpellName(spell);--FW:Show(spell);
			start,duration,enabled = GetSpellCooldown(i, "player");
			--[[
			if spell == "Воскрешение" then
				print("T2", start,duration,enabled, basecd, spellid, BanSpells[spellid])
			end
			]]
			...
			--[[	
				if maxCharges then
					...
					if ( maxCharges and ( cooldownStart > 0 ) and ( cooldownDuration > 1.5 ) ) then
						if ( maxCharges ~= currentCharges) then
				--			print("CD_ScanBookCooldowns YES max charge", cooldownStart, cooldownDuration, spell, enabled)
							CD:CheckCooldown(spell,cooldownStart,cooldownDuration*(maxCharges-currentCharges),texture,(CD_IsSpecialCooldown(spell)) or FLAG_SPELL, currentCharges);
						end
					elseif ( start > 0 and duration > 1.5 ) then -- Number - 0 if the spell is active (Stealth, Shadowmeld, Presence of Mind, etc) and the cooldown will begin as soon as the spell is used/cancelled; 1 otherwise. 
					--	print("CD_ScanBookCooldowns No max charge", start, duration, spell, enabled)
	--				==	assert(false, "Error")
						CD:CheckCooldown(spell,start,duration,texture,(CD_IsSpecialCooldown(spell)) or FLAG_SPELL);
					else
					--	print("T", enabled, start, duration, spell)
						CD:CheckCooldown(spell,start,duration,texture,(CD_IsSpecialCooldown(spell)) or FLAG_SPELL);
					end
				end
			i=i+1;
	...
	while true do  NOT WORKING AT ALL
		...
		if spell then
			...
			start,duration,enabled = GetSpellCooldown(i);
			if enabled == 1 then
				--FW:Show("moo");
				CD:CheckCooldown(spell,start,duration,texture,(CD_IsSpecialCooldown(spell)) or FLAG_PET);
			end
