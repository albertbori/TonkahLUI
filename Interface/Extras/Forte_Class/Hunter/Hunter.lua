...
if FW.CLASS == "HUNTER" then
	...
	if ST then
		...
		do
			...
			local freezing_trap = FW:SpellName(1499);			
			...
			local function HT_CombatLogEvent(event,...)
				if select(5,...) == PLAYER then
					if select(2,...) == "SPELL_AURA_APPLIED" then
						...
						elseif select(13,...) == freezing_trap then 
							local i = ST.ST:find(freezing_trap,8);
							 if i and ST.ST[i][15] == 1 then 
								 ST.ST[i][1] = GetTime()+0; 
								 ST.ST[i][14] = 0; 
								 ST.ST[i][15] = 0; -- use this to set as already triggered 
								 ST.ST[i][12] = 0; -- reset the fade event on refresh end end
							 end
